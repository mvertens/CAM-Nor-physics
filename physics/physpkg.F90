...
!=======================================================================
  ...
  subroutine phys_init( phys_state, phys_tend, pbuf2d, cam_in, cam_out )
   ...
   !call convect_diagnostics_init
    call convect_diagnostics_init(pbuf2d) !-> cam6_3_102
...
!+tht I put this inside if-block since call before dme_adjust is incorrect for the moist dycores
    !
...
!-tht
      ...
      if (thermo_budget_history) then
       ...
       !call set_dry_to_wet(state) !tht: c'd out from 100->106 (WOW!...)
...
!     call physics_dme_adjust(state, tend, qini, ztodt)
      call physics_dme_adjust(state, tend, qini, totliqini, toticeini, ztodt, eflx, dsema, &
                              ohf_adjust, cam_in%ocnfrac, cam_in%sst, cam_in%ts) !+tht
!+tht call this irrespective of thermo_budget_history (as in pel cam6_3_106) but after dme_adjust (as in pel cam6_3_100)
      call cam_thermo_water_update(state%q(:ncol,:,:), lchnk, ncol, vc_dycore, &
           to_dry_factor=state%pdel(:ncol,:)/state%pdeldry(:ncol,:))
!-tht
